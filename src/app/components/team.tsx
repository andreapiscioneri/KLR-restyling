...
export function Team({ go }: { go: (r: Route) => void }) {
  const leaders = leadershipOrder
    .map((id) => leadership.find((p) => p.id === id))
    .filter((p): p is (typeof leadership)[number] => Boolean(p));
      ...
      <section className="relative pt-28 md:pt-32 pb-20 md:pb-24 overflow-hidden" style={{ background: G.yellow }}>
        ...
        <div className="max-w-6xl mx-auto px-8">
          <AnimatedSection>
            ...
            <div className="mt-12 grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-5">
              {leaders.map((p) => (
                <article
                  key={p.id}
                  className="group rounded-[24px] p-4 md:p-5 border border-[#2E2784]/15 bg-[#F8AE01] text-center"
                  style={softShadow}
                >
                  <div className="mx-auto w-24 h-24 md:w-28 md:h-28 rounded-full overflow-hidden border-[4px] border-white bg-white">
                    <ImageWithFallback src={p.img} alt={p.name} className="w-full h-full object-cover" />
                  </div>

                  <h3 className="text-[#2E2784] tracking-[-0.02em] mt-4" style={{ fontSize: "1.05rem", lineHeight: 1.2, fontWeight: 800 }}>
                    {p.name}
                  </h3>
                  <p className="text-[#2E2784]/85 tracking-tight mt-2" style={{ fontSize: "0.9rem", lineHeight: 1.35, minHeight: "2.6rem" }}>
                    {p.role}
                  </p>
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-[#2E2784] hover:text-black transition-colors"
                      style={{ fontSize: "0.78rem", fontWeight: 700 }}
                    >
                      Profile <ArrowUpRight className="w-3.5 h-3.5" />
                    </button>
                    <a
                      href={linkedinById[p.id]}
                      target="_blank"
                      rel="noreferrer"
                      className="w-8 h-8 rounded-full bg-[#2E2784] text-white flex items-center justify-center hover:bg-black transition-colors"
                      aria-label={`LinkedIn profile for ${p.name}`}
                    >
      ...
      <section className="relative pt-28 md:pt-32 pb-20 md:pb-24 overflow-hidden" style={{ background: G.yellow }}>
        ...
        <div className="max-w-6xl mx-auto px-8">
          <AnimatedSection>
            ...
            {/* OPEN ROLES CARDS — blue with white text */}
