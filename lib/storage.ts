...
const MAPPERS: Record<string, Mapper> = {
  posts: {
    table: "posts",
    order: "position ASC",
    columns: ["id","slug","title","date","excerpt","img","link","category","content_html","public_preview","cornerstone","author_name","author_avatar","status","position","title_html","summary_eyebrow","summary_title","layout_mode","layout_blocks","updated_at"],
    toRow: (p, i, now) => ({
      ...
      status: p.status ?? "published", position: i,
      title_html: p.titleHtml ?? null,
      summary_eyebrow: p.summaryEyebrow ?? null,
      summary_title: p.summaryTitle ?? null,
      layout_mode: p.layoutMode ?? null,
      layout_blocks: p.layoutBlocks ? json.stringify(p.layoutBlocks) : null,
      updated_at: now,
    }),
    fromRow: (r) => ({
      ...
      summaryEyebrow: r.summary_eyebrow ?? undefined,
      summaryTitle: r.summary_title ?? undefined,
      titleHtml: r.title_html ?? undefined,
      layoutMode: r.layout_mode ?? undefined,
      layoutBlocks: r.layout_blocks ? json.parse(r.layout_blocks, []) : undefined,
    }),
  },
